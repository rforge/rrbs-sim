...
if(num.dm.regs<nrow(diff.regs)){dm.rows=sample(1:nrow(diff.regs),num.dm.regs)}
else{dm.rows=1:nrow(diff.regs)
num.dm.regs=nrow(diff.regs)}
dmsites=rep(0,nsites)
diff.regs.samp=diff.regs[dm.rows,]
...
##Variogram model parameters for sim.corr.pm
pm.vfit.chr11.pars=c(0.680,0.151,0.014,1594.341)
...
covmat.curr=CovMatrix(pos.curr,model="gauss",param=pm.vfit.chr11.pars)
corrmat.curr=covmat.curr/covmat.curr[1,1]
